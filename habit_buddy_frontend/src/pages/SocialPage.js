      ...
      <div className="card">
        <div className="card-header">
          <div>
            ...
            <p className="card-subtitle">Accountability with people you trust</p>
          </div>
        ...
        <div className="card-body">
          <div className="grid" style={{ gap: 12 }}>
            ...
            <div className="notice">
              Social graph will be backed by real endpoints later; for now, you can demo requests and accept/decline incoming invites.
            </div>

            <div className="grid" style={{ gap: 10 }}>
              {incoming.length > 0 ? (
                <div className="notice warn">Incoming requests: {incoming.length}</div>
              ) : (
                <div className="notice">No incoming requests.</div>
              )}
            ...
            <div className="grid" style={{ gap: 10, marginTop: 4 }}>
              <div style={{ fontWeight: 700, fontSize: 13 }}>Connected</div>
              {accepted.map((f) => (
              ...
              {accepted.length === 0 ? <div className="notice">No friends yet. Add one above.</div> : null}
            </div>

            <div className="grid" style={{ gap: 10, marginTop: 4 }}>
              <div style={{ fontWeight: 700, fontSize: 13 }}>Outgoing</div>
              {outgoing.map((f) => (
                <div key={f.id} className="list-item">
                  <div>
                    <h3>{f.name}</h3>
              ...
              {outgoing.length === 0 ? <div className="notice">No outgoing requests.</div> : null}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="list">
            ...
            {demoApi.listMyBadges().length === 0 ? <div className="notice">No badges yet.</div> : null}
          </div>
