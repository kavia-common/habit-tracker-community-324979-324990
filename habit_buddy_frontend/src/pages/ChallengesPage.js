...
export default function ChallengesPage() {
  ...
  return (
    <div className="grid" style={{ gap: 12 }}>
      <PageHeader
        title="Challenges"
        subtitle="Create sprints, invite others, and track progress together."
        actions={
          <button type="button" className="btn btn-small" onClick={() => ui.showToast("Challenge discovery coming soon (demo)")}>
            Discover
          </button>
        }
      />
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">Clear goals, simple scoring</p>
            </div>
          </div>
          <div className="card-body">
            <div className="grid" style={{ gap: 12 }}>
              ...
              <div className="grid cols-2" style={{ gap: 12 }}>
                ...
                <FormField label="Goal value" help="Days for streak; numeric target for value goals.">
                  <input
                    className="input"
                    type="number"
                    min={1}
              ...
              <div className="row between" style={{ alignItems: "center" }}>
                <span style={{ color: "var(--muted)", fontSize: 12 }}>Demo: stored locally for now.</span>
                <button
                  type="button"
              ...
              <div className="notice">
                Later this will map to <code>/challenges</code> and <code>/challenges/:id/join</code>.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="card-body">
            <div className="list">
              {challenges.map((c) => {
                ...
                const pct = goal > 0 ? Math.min(100, Math.round((daysCompleted / goal) * 100)) : 0;
                  ...
                  <div key={c.id} className="list-item">
                    <div style={{ maxWidth: "100%" }}>
                      <h3 style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        {joined ? <span className="pill">Joined</span> : null}
                      </h3>
                      <p>{c.description || "—"}</p>
                      <p style={{ marginTop: 6 }}>
                        Goal: <strong>{c.goal_type}</strong> • Value: <strong>{c.goal_value ?? "—"}</strong> • Participants: <strong>{c.participants || 0}</strong>
                      </p>
              ...
              {challenges.length === 0 ? (
                <EmptyState title="No challenges yet" description="Create one on the left to kick off a sprint." />
              ) : null}
            </div>
          </div>
        </div>
