...
export default function HabitsPage() {
  ...
  // Shared draft across list items (demo). Keep controls visually tight.
  ...
  return (
    <div className="grid" style={{ gap: 12 }}>
      <PageHeader
        title="Habits"
        subtitle="Create habits, check in daily, and build streak momentum."
        actions={
          <button type="button" className="btn btn-small" onClick={() => ui.showToast("Habit templates are in Power Features (demo)")}>
            Use template
          </button>
        }
      />
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">Targets, reminders, and visibility</p>
            </div>
          </div>
            ...
            <div className="grid" style={{ gap: 12 }}>
              ...
              <div className="grid cols-2" style={{ gap: 12 }}>
                <FormField label="Habit type">
                  <select className="select" value={draft.habit_type} onChange={(e) => setDraft((d) => ({ ...d, habit_type: e.target.value }))}>
                ...
                <FormField label="Reminder time" help="Optional (demo).">
                  <input className="input" type="time" value={draft.reminder_time} onChange={(e) => setDraft((d) => ({ ...d, reminder_time: e.target.value }))} />
                </FormField>
              </div>
              ...
              <div className="grid cols-2" style={{ gap: 12 }}>
                ...
                <FormField label="Unit">
                  <input className="input" value={draft.unit} onChange={(e) => setDraft((d) => ({ ...d, unit: e.target.value }))} placeholder="e.g., glasses / min / reps" />
                </FormField>
              </div>
              ...
              <div className="row between">
                <div>
                  <div style={{ fontWeight: 800, fontSize: 13 }}>Public habit</div>
                  <div style={{ color: "var(--muted)", fontSize: 12 }}>Allow sharing to feed/groups (demo)</div>
                </div>
                <button type="button" className={`btn btn-small ${draft.is_public ? "btn-primary" : ""}`} onClick={() => setDraft((d) => ({ ...d, is_public: !d.is_public }))}>
                  {draft.is_public ? "On" : "Off"}
                </button>
              </div>

              <div className="row between" style={{ alignItems: "center" }}>
                <span style={{ color: "var(--muted)", fontSize: 12 }}>Stored in demo state for now.</span>
                <button
                  type="button"
              ...
              <div className="notice">
                Habit upgrades map cleanly to backend fields (type/target/unit/reminder/public) when APIs are enabled.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="list">
              ...
              {habits.length === 0 ? (
                <EmptyState title="No habits yet" description="Create your first habit to start tracking daily check-ins." />
              ) : null}
            </div>
