  ...
  return (
    <div className="grid cols-2">
      <div className="card">
        <div className="card-header">
          <div>
            ...
            <p className="card-subtitle">Make Habit Buddy feel like yours</p>
          </div>
        </div>
          ...
          <div className="grid" style={{ gap: 12 }}>
            <FormField label="Motivation style" help="Changes how tips/messages are phrased (demo).">
              <select
                ...
                onChange={(e) => setPrefs((p) => ({ ...p, motivationStyle: e.target.value }))}
              >
            ...
            <FormField label="Dashboard layout">
              <select
                ...
                onChange={(e) => setPrefs((p) => ({ ...p, dashboardLayout: e.target.value }))}
              >
            ...
            <FormField label="Weekly goal (days)">
              <input
                className="input"
                type="number"
                min={1}
            ...
            <div className="row between">
              <div>
                <div style={{ fontWeight: 700, fontSize: 13 }}>Reduce motion</div>
                <div style={{ color: "var(--muted)", fontSize: 12 }}>Accessibility preference (demo)</div>
              </div>
              <button type="button" className="btn btn-small" onClick={() => setPrefs((p) => ({ ...p, reduceMotion: !p.reduceMotion }))}>
            ...
            <div className="notice">
              These settings currently persist locally. Later they’ll be synced to server settings APIs.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {prefs.motivationStyle === "encouraging" ? (
            <div className="notice">You’re doing great — just one small step today.</div>
          ) : null}
